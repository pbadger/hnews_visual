...
output = open("hn_thurs_18_1130pm.csv", "wb")
...
def get_post_attrs(dom,post_index):
  ...
  if 'hour' in time_string:
    ...
    if min_since_post < .5:
      ...
      hours_since_post = 1
  return [title,url,hours_since_post,time_string]
...
def get_keywords(article_url,title):
  try:
    ...
    top_word_number = int(word_count.most_common(1)[0][1]*.75)
...
def print_to_csv(article_attributes):
  ...
  encoded_url = url.encode('ascii','ignore')
...
while end_scrape == False:
  ...
  attributes_1 = get_attributes(dom,1);
  print attributes_1[3]
  attributes_30 = get_attributes(dom,30);
  print attributes_30[3]

  for post_index in range(1,31):
    
    attributes = get_attributes(dom,post_index);
    title = attributes[0]
    ...
    if hours_since_post < 24:
      ...
      print_to_csv(article_attributes)
